...
file static class SearchCommand
{
        ...
        IOptions<EmojiSettings> emojiSettings)
    {
        ...
        var categories = await cache.GetGuildCategoriesAsync(guildId, client);
        var rankedMaps = await client.RankedMaps.GetAsync(contextId, search, pageOption, CancellationToken.None);
        if (!rankedMaps.TryGetValue(out var pagedRankedMaps, out var error))
            return builder.WithTextDisplay($"Error fetching ranked maps: {error}");
        ...
        foreach (var rankedMap in pagedRankedMaps.Data)
        {
            ...
            builder.WithContainer(mapContainerBuilder);
        }
