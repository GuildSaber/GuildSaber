...
public partial class ScoringTeamModuleSlash
{
    ...
    public async Task AdminConfirmation(ContextId contextId, int page)
    {
        ...
        var component = ScoringTeamMenuView.BuildComponent(pendingScores.Data, categories, EmojiSettings)
            .WithActionRow(x => x
        ...
        IOptions<EmojiSettings> emojiSettings)
    {
        ...
        foreach (var (i, version) in rankedMap.Versions.Index())
        {
            ...
            if (rankedScore.PrevScore is { } prevScore)
            {
                ...
                sb.Append(" (").Append(diff >= 0 ? "+" : "").Append(diff.ToString("N0")).AppendLine(") ");
            }

            sb.AppendLine(TimestampTag.FormatFromDateTimeOffset(score.SetAt, TimestampTagStyles.ShortDateTime));

            var mods = rankedScore.Score.Modifiers;
            mods |= RankedMapRequests.EModifiers.DisappearingArrows;
            mods |= RankedMapRequests.EModifiers.FasterSong;

            if (mods != RankedMapRequests.EModifiers.None)
                sb.Append("Mods: **").Append(mods.ToModifiersString()).AppendLine("**");

            if (true)
                sb.Append("Pause duration: **12s**");

            if (score.IsFullCombo)
                sb.Append(" | **FC**");
            else
                sb.Append(" | Misses: **").Append(score.MissedNotes).Append("**");

            sb.AppendLine(score is RankedScoreResponses.Score.BeatLeaderScore
            {
            ...
            mapContainerBuilder.WithSection(x => x
                .WithTextDisplay("### [TECH_Sage](https://beatleader.com/u/76561198134068431)\n" +
                                 sb)
                .WithAccessory(new ThumbnailBuilder()
