...
const GuildMapRow = ({ map }: Props) => {
  ...
  return (
    <div className="grid grid-cols-[auto_1fr_auto] items-center gap-4 py-2 sm:grid-cols-[auto_1fr_auto_auto]">
      ...
      <div>
        <p className="line-clamp-2 break-all">{song.info.name}</p>
        <p className="text-sm">
      ...
      <div className="flex flex-col items-end justify-end gap-1">
        <div className="flex h-7 items-center gap-1 rounded border border-amber-800 bg-amber-800/10 p-1 px-2 text-sm text-amber-800 dark:border-amber-400 dark:bg-amber-400/20 dark:text-amber-400">
          {map.rating.diffStar} <Star className="size-4" />
        </div>
        <div className="flex h-7 items-center gap-1 rounded border border-teal-800 bg-teal-800/10 p-1 px-2 text-sm text-teal-800 dark:border-teal-400 dark:bg-teal-400/20 dark:text-teal-400">
